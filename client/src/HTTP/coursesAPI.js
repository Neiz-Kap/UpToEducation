...
export const fetchCourses = async (occupationId, authorId, page = 1, limit = 10) => {
  const { data } = await $host.get('api/card', {
    params: {
      occupationId, authorId, page, limit
    }
  })
  return data
}
...
export const fetchDevices = async (occupationId, authorId, page = 1, limit = 10) => {
  const { data } = await $host.get('api/card', {
    params: {
      occupationId, authorId, page, limit
    }
...
// ready
export const createDevice = async (card) => {
  const { data } = await $authHost.post('api/card', card) // ready
  return data
}
